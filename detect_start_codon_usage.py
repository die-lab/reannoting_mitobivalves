...
#like the script for make a table with every lenght, but with start codons.
...
for gene in mito_genes:
	...
	for file in gene_files:
		record = SeqIO.read(file,"fasta")
		count = 0
		start_tmp = record.seq[0:3]
		if start tmp in alt_codons:
			tab_dict[record.id][gene] = start_tmp
		else:
			tab_dict[record.id][gene] = 'XXX'
